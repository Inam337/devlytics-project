...
describe('PeriodUtil', () => {
  it('resolves a monthly window to the first and last day of that month', () => {
    const { start, end } = PeriodUtil.resolve(
      'MONTHLY',
  ...
  it('computes the previous period relative to the current one', () => {
    const current = PeriodUtil.resolve(
      'MONTHLY',
      new Date('2026-03-10T00:00:00Z'),
    );
    const previous = PeriodUtil.previous(
      'MONTHLY',
      new Date('2026-03-10T00:00:00Z'),
    );
  ...
  it('enumerates every UTC day in an inclusive range', () => {
    const days = PeriodUtil.eachDay(
      new Date('2026-01-01'),
      new Date('2026-01-03'),
    );
    expect(days.map((d) => PeriodUtil.toDateOnly(d))).toEqual([
      '2026-01-01',
      '2026-01-02',
      '2026-01-03',
    ]);
