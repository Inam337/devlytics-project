...
const OPEN_ISSUE_STATUSES: QualityIssueStatus[] = [
  'OPEN',
  'ACKNOWLEDGED',
  'IN_PROGRESS',
];
...
function toDecimalNumber(value: unknown): number | null {
  return value === null || value === undefined
    ? null
    : NumberUtil.toNumber(value as Prisma.Decimal);
}
...
export class ImprovementInsightsService {
  ...
  async dashboard(organizationId: string, query: InsightsScopeQueryDto) {
    ...
    const repositoryIds = await this.metricCalculation.resolveRepositoryIds(
      organizationId,
      {
        teamId: query.teamId,
        projectId: query.projectId,
    ...
    const experimentBase: Prisma.EngineeringExperimentWhereInput = {
      organizationId,
      ...experimentScope,
    };
    ...
    const [
      ...
      this.prisma.engineeringExperiment.count({
        where: { ...experimentBase, status: 'ACTIVE' },
      }),
      this.prisma.engineeringExperiment.count({
        where: { ...experimentBase, status: 'PROVEN' },
      }),
      this.prisma.engineeringExperiment.count({
        where: { ...experimentBase, status: { in: ['PROVEN', 'FAILED'] } },
      }),
    ...
    const topProblemsRaw = topProblemsGroups
    ...
    const trendProofs = await this.prisma.improvementProof.findMany({
      where: {
        organizationId,
        verifiedAt: {
          gte: PeriodUtil.addDays(start, -30 * (TREND_MONTHS - 1)),
        },
        experiment: experimentScope,
      },
    ...
    return {
      summary: {
        ...
        improvementRate: completedTotal
          ? NumberUtil.percent(provenCount, completedTotal)
          : 0,
      },
      activeExperiments: activeExperiments.map((experiment) => ({
        ...
        primaryMetric: experiment.metrics[0]
          ? {
              ...
              baselineValue: toDecimalNumber(
                experiment.metrics[0].baselineValue,
              ),
      ...
      topProblems: topProblemsRaw.map((row) => ({
        category: row.category,
        openCount: row._count,
      })),
      recommendations,
      ...
      items.map((experiment) => ({
        ...
        result: experiment.proof
          ? {
              ...
              improvementPercentage: toDecimalNumber(
                experiment.proof.improvementPercentage,
              ),
