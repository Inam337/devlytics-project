...
export class ImprovementInsightsController {
  ...
  @ApiOperation({ summary: 'Improvement Engine dashboard: active experiments, top problems, recent proofs, trend' })
  dashboard(@OrganizationId() organizationId: string, @Query() query: InsightsScopeQueryDto) {
    return this.insights.dashboard(organizationId, query);
  }

  @Get('problems')
  @RequirePermissions(Permission.IMPROVEMENT_READ)
  @ApiOperation({ summary: 'Detected engineering problems, derived from open quality issues' })
  problems(@OrganizationId() organizationId: string, @Query() query: ProblemsQueryDto) {
    return this.insights.problems(organizationId, query);
  }

  @Get('history')
  @RequirePermissions(Permission.IMPROVEMENT_READ)
  @ApiOperation({ summary: 'Completed improvement experiments with their verified results' })
  history(@OrganizationId() organizationId: string, @Query() query: HistoryQueryDto) {
