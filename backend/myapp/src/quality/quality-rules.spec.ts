...
describe('quality-rules', () => {
  ...
  it('flags low test coverage proxy below the 60% reference', () => {
    const findings = evaluateRules(
      evidence({ testFileChanges: 10, filesChanged: 100 }),
    );
    const finding = findings.find(
      (f) => f.ruleId === 'RULE_LOW_TEST_COVERAGE_PROXY',
    );
  ...
  it('escalates severity as the CI failure rate worsens', () => {
    const minor = evaluateRules(
      evidence({ totalBuilds: 100, failedBuilds: 15 }),
    );
    const critical = evaluateRules(
      evidence({ totalBuilds: 100, failedBuilds: 50 }),
    );

    expect(
      minor.find((f) => f.ruleId === 'RULE_CI_FAILURE_RATE')?.severity,
    ).toBe('MINOR');
    expect(
      critical.find((f) => f.ruleId === 'RULE_CI_FAILURE_RATE')?.severity,
    ).toBe('CRITICAL');
  });
  ...
  it('flags a high share of large pull requests', () => {
    const findings = evaluateRules(
      evidence({ totalPullRequests: 10, largePullRequests: 7 }),
    );
    const finding = findings.find(
      (f) => f.ruleId === 'RULE_LARGE_PULL_REQUESTS',
    );
  ...
  it('flags low documentation activity relative to commit count', () => {
    const findings = evaluateRules(
      evidence({ commits: 100, docFileChanges: 2 }),
    );
    const finding = findings.find(
      (f) => f.ruleId === 'RULE_LOW_DOCUMENTATION_RATIO',
    );
  ...
  it('flags commits that touch an unusually wide file span', () => {
    ...
    const finding = findings.find(
      (f) => f.ruleId === 'RULE_HIGH_COMMIT_FILE_SPAN',
    );
  ...
  it('every finding names the rule that produced it and its raw measured value', () => {
    const findings = evaluateRules(
      evidence({ testFileChanges: 5, filesChanged: 100 }),
    );
