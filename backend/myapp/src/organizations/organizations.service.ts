...
import { ROLE_DEFINITIONS, ROLE_PERMISSIONS } from '../common/constants/permissions';
import { AppException } from '../common/exceptions/app.exception';
import { PrismaService } from '../database/prisma.service';
import { DEFAULT_SCORING_WEIGHTS, SCORE_CATEGORIES } from '../scoring/scoring.constants';
...
export class OrganizationsService {
  ...
  async create(dto: CreateOrganizationDto, actor: ActorContext = {}): Promise<Organization> {
    ...
    const organization = await this.prisma.$transaction(async (tx) => {
      const created = await tx.organization.create({
        data: {
          ...
          onboardingState: { currentStep: 1, completedSteps: [], skipped: false },
        },
      });
  ...
  async provision(
    ...
    this.logger.log(`Provisioned roles, weights and local AI provider for organization ${organizationId}`);
  }
  ...
  async findOne(organizationId: string, requestedId: string) {
    ...
    if (organizationId !== requestedId) {
      throw AppException.forbidden('You do not have access to this organization');
    }
  ...
  async update(
    ...
    const slug = dto.slug && dto.slug !== before.slug ? await this.resolveSlug(dto.slug) : undefined;
  ...
  private async resolveSlug(source: string): Promise<string> {
    ...
    while (await this.repository.findBySlug(candidate)) {
      ...
      if (suffix > 999) throw AppException.conflict('Could not derive a unique organization slug');
    }
    return candidate;
  }
}
